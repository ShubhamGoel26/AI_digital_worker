...
class ScreenshotHandler(logging.Handler):
	...
	async def capture_screenshot(self, filename):
		...
		with mss() as sct:
			# Optionally focus on browser region (adjust coordinates)
	...
	def emit(self, record):
		...
		triggers = ['Action 1/1:', 'Next goal:', 'Searched for', 'Navigating to', 'Entering', 'Clicking', 'User logged in']
		if any(trigger in message for trigger in triggers):
			...
			try:
				...
				asyncio.get_event_loop().run_until_complete(self.capture_screenshot(filename))
				log_entry = {
					'step': self.step,
					'action': message
					if any(
						t in message for t in ['Action', 'Navigating', 'Entering', 'Clicking', 'Searched for', 'User logged in']
					)
					else '',
...
async def main():
	...
	async with async_playwright() as p:
		...
		try:
			...
			agent.task = 'Navigate to https://console.aws.amazon.com/ and wait for the login page to load'
			await agent.run()
