...
async def main():
    ...
    try:
        ...
        pages = first_agent.browser_context.pages
        if not pages:
            page = await first_agent.browser_context.new_page()
        else:
            page = pages[0]
    ...
    try:
        while True:
            ...
            # Log browser state after run
            try:
                pages = fresh_agent.browser_context.pages
                if not pages:
                    page = await fresh_agent.browser_context.new_page()
                else:
                    page = pages[0]
                current_url = await page.evaluate("() => window.location.href")
                with open(llm_log_file, "a", encoding="utf-8") as f:
