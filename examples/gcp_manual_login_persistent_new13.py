...
# -----------------------------------------------------------------
# 5.  Helper to serialize AgentHistoryList
# -----------------------------------------------------------------
def serialize_history(history):
	"""Convert AgentHistoryList to a serializable dictionary."""
	try:
		# Assume history is a list-like object with serializable items
		...
		for item in history:
			if hasattr(item, '__dict__'):
				...
				serializable_history.append(str(item))  # Fallback to string representation
...
async def main():
	...
	# -------------------------------------------------------------
	# 6‑A  First agent: open GCP sign‑in page
	# -------------------------------------------------------------
	first_agent = Agent(
	...
	with open(llm_log_file, 'a', encoding='utf-8') as f:
		...
		f.write(json.dumps(serialize_history(history), indent=2))
		f.write('\n')
		f.flush()
