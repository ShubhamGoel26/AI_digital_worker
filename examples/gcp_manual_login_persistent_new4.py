...
class ScreenshotHandler(logging.Handler):
	...
	async def _cap(self, fn):
		...
		with mss() as sct:
			...
			img = Image.frombytes('RGB', (mon['width'], mon['height']), sct.grab(mon).rgb)
			img.save(fn, 'PNG')
	...
	def emit(self, record):
		...
		with open(self.debug_file, 'a', encoding='utf-8') as f:
			f.write(f'DEBUG: {msg}\n')
		if any(k in msg for k in ['Action 1/1', 'Next goal', 'Clicked', 'User logged in']):
			fn = f'{self.ss_dir}/step_{self.step}_{datetime.now():%Y%m%d_%H%M%S}.png'
			asyncio.create_task(self._cap(fn))
			self.step += 1
...
async def main():
	...
	# ---- Login phase ------------------------------------------------------
	login_agent = Agent(
		task='Navigate to Google Cloud Platform and go to sign in', llm=EXEC_LLM, close_browser_on_run=False, enable_memory=False
	)
