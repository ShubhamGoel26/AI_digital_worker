...
async def main():
	...
	try:
		current_url = await first_agent.browser_context.pages[0].evaluate('() => window.location.href')
		with open(llm_log_file, 'a', encoding='utf-8') as f:
	...
	try:
		while True:
			...
			# Log browser state after run
			try:
				current_url = await fresh_agent.browser_context.pages[0].evaluate('() => window.location.href')
				with open(llm_log_file, 'a', encoding='utf-8') as f:
					f.write(f'\n=== Browser State After Run: URL = {current_url} ===\n')
					f.flush()
			except Exception as e:
