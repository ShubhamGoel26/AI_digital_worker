#!/usr/bin/env python
"""
GCP automation script for independent tasks
--------------------------------
1. Loads independent tasks from tasks.xlsx.
2. Runs a first “login” agent for manual GCP sign-in.
3. For each task: navigates to Google Cloud Console "Home" page before execution.
4. Saves per-task logs/screenshots and generates a summary report.
...
load_dotenv()
BASE_DIR = Path('runs\Gemini_new_independent')
...
# AdministratorAgent for Independent Tasks
class IndependentAdministratorAgent:
	"""Runs independent tasks sequentially, navigating to "Home" before each task."""

	def __init__(self, tasks: List[Task], browser, browser_context, base_dir: Path):
		self.tasks = [task for task in tasks if task.prerequisite is None]
	...
	async def run(self):
		...
		# Write summary.json
		summary_path = self.base_dir / 'independent_summary.json'
		summary_path.write_text(
			json.dumps([asdict(r) for r in self.task_reports], indent=2, ensure_ascii=False),
			encoding='utf-8',
		)
		# Print console summary
		print('\n──────────  INDEPENDENT TASKS EXECUTION SUMMARY  ──────────')
	...
	async def _run_one(self, task: Task, idx: int):
		...
		task_dir = self.base_dir / f'independent_task_{task.number:02d}_{safe}_{ts}'
		...
		lg = logging.getLogger(f'independent_task_{task.number}')
		...
		lg.info(f'Independent Task {task.number}: {task.description}')

		# Modify task description to include navigation to "Home" page
		effective_description = f"Navigate to Google Cloud Console 'Home' page and then {task.description}"

		# Create agent with modified description
...
# Main driver for Independent Tasks
async def main():
	shared_ctx = None
	shared_browser = None
	try:
		...
		run_dir = BASE_DIR / f'independent_run_{run_ts}'
		...
		run_logger = logging.getLogger('independent_initial_run')
		...
		# Load tasks and run with IndependentAdministratorAgent
		tasks_file = Path(__file__).parent / 'tasks.xlsx'
		tasks = load_tasks(tasks_file)
		admin = IndependentAdministratorAgent(
			tasks=tasks,
