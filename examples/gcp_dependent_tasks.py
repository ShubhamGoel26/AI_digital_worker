#!/usr/bin/env python
"""
GCP automation script for dependent tasks
--------------------------------
1. Loads dependent tasks from tasks.xlsx.
2. Runs a first “login” agent for manual GCP sign-in.
3. Executes tasks sequentially without navigating back to console.
...
load_dotenv()
BASE_DIR = Path('runs\Gemini_new_dependent')
...
# AdministratorAgent for Dependent Tasks
class DependentAdministratorAgent:
	"""Runs dependent tasks sequentially without navigating back to console."""

	def __init__(self, tasks: List[Task], browser, browser_context, base_dir: Path):
		self.tasks = [task for task in tasks if task.prerequisite is not None]
	...
	async def run(self):
		...
		# Write summary.json
		summary_path = self.base_dir / 'dependent_summary.json'
		summary_path.write_text(
			json.dumps([asdict(r) for r in self.task_reports], indent=2, ensure_ascii=False),
			encoding='utf-8',
		)
		# Print console summary
		print('\n──────────  DEPENDENT TASKS EXECUTION SUMMARY  ──────────')
	...
	async def _run_one(self, task: Task, idx: int):
		...
		task_dir = self.base_dir / f'dependent_task_{task.number:02d}_{safe}_{ts}'
		...
		lg = logging.getLogger(f'dependent_task_{task.number}')
		...
		lg.info(f'Dependent Task {task.number}: {task.description}')

		# Use original task description for dependent tasks
		effective_description = task.description

		# Create agent with original description
...
# Main driver for Dependent Tasks
async def main():
	shared_ctx = None
	shared_browser = None
	try:
		...
		run_dir = BASE_DIR / f'dependent_run_{run_ts}'
		...
		run_logger = logging.getLogger('dependent_initial_run')
		...
		# Load tasks and run with DependentAdministratorAgent
		tasks_file = Path(__file__).parent / 'tasks.xlsx'
		tasks = load_tasks(tasks_file)
		admin = DependentAdministratorAgent(
			tasks=tasks,
